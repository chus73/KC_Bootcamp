/* Copyright 2018 FXCM Global Services, LLC
...
*/
package main.scala.com.bddprocessing.FX.GetHistoricPrices;
...
public class Main {
    ...
    public static void getHistoryPrices(IPriceHistoryCommunicator communicator, String sInstrument,
        ...
        if (response != null) {
            //printPrices(communicator, response);//comentado para que no intente mostrar los datos en consola
            createList(communicator, response, sInstrument);//Creado por Kenny
        }
    }
    ...
    /**
     * Create list.  Created by Kenny to collect the data in one single file.
     */
    public static void createList(IPriceHistoryCommunicator communicator, IPriceHistoryCommunicatorResponse response, String kInstrument)  throws Exception {
        // use O2GMarketDataSnapshotResponseReader to extract price data from the response object
        O2GMarketDataSnapshotResponseReader reader = communicator.createResponseReader(response);

        //List<String[]> out_list = new ArrayList<String[]>();
        //String[] out_list=null;
        File file = new File("E:\\Nube\\Google Drive\\DataScience\\proyectoFinal\\fx"+ kInstrument.replace("/","") + ".csv");

        FileWriter outputfile = new FileWriter(file);
        CSVWriter writer = new CSVWriter(outputfile, ',',CSVWriter.NO_QUOTE_CHARACTER);
        String[] out_list={"DateTime","BidOpen","BidHigh","BidLow","BidClose","AskOpen","AskHigh","AskLow","AskClose","Volume"};
        writer.writeNext(out_list);
        for (int i = 0; i < reader.size(); i++) {
            //if (reader.isBar()) {
            //    System.out.println(String.format("DateTime=%s, BidOpen=%s, BidHigh=%s, BidLow=%s, BidClose=%s, AskOpen=%s, AskHigh=%s, AskLow=%s, AskClose=%s, Volume=%s",
            writer.writeNext(new String[] {mDateFormat.format(reader.getDate(i).getTime()) , String.valueOf(reader.getBidOpen(i)) , String.valueOf(reader.getBidHigh(i))
                    , String.valueOf(reader.getBidLow(i)), String.valueOf(reader.getBidClose(i)) , String.valueOf(reader.getAskOpen(i)), String.valueOf(reader.getAskHigh(i))
                    , String.valueOf(reader.getAskLow(i)), String.valueOf(reader.getAskClose(i)),  String.valueOf(reader.getVolume(i) ) });
